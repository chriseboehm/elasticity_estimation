...
f(1) = px_tm1_i*phi_i^(1-1/psi)*x^(1/psi)*mu*...
    (K^alpha*L^(1-alpha))^((psi-1)/psi)*(1-alpha) - w_tm1*L;

f(2) = px_tm1_i*phi_i^(1-1/psi)*x^(1/psi)*mu*...
    (K^alpha*L^(1-alpha))^((psi-1)/psi)*alpha - R_tm1*K;

M = int_agg(mnJ,mJ);

f(3) = px_tm1_i*phi_i^(1-1/psi)*x^(1/psi)*(1-mu)*M^(1/om-1/psi)...
    *nu_i*mnJ^(-1/om) - pnJ_tm1_i;

f(4) = px_tm1_i*phi_i^(1-1/psi)*x^(1/psi)*(1-mu)*M^(1/om-1/psi)...
